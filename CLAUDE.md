...
-   **Command hierarchy**:
    ...
    -   `CommandEncryption` - Commands requiring unlocked encryption wallet
    -   `CommandSecured` - Commands requiring unlocked user wallet
...
-   TUI can be used in environment where Beekepeer is already running (`CLIVE_BEEKEEPER__REMOTE_ADDRESS` and
...
-   Beekeeper wallets are stored in the `$CLIVE_DATA_PATH/beekeeper` directory
...
-   `beekeeper` - Async beekeeper instance (spawned automatically)
