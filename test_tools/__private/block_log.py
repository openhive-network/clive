...
class BlockLog:
    ...
    def __run_and_get_output(self, *args: str) -> str:
        # print(f"running: block_log_util {" ".join(args)}")
        process = subprocess.run(
    ...
    def get_head_block_number(self, log_file: Path | None = None) -> int:
        """
        ...
        log_path_arg = ["--log-path", str(log_file) + "_get_head_block_number"] if log_file else []
        return int(self.__run_and_get_output("--get-head-block-number", *block_log_arg, *log_path_arg))

    def get_block(self, block_number: int, log_file: Path | None = None) -> BlockLogUtilResult:
        """
        ...
        log_path_arg = ["--log-path", str(log_file) + "_get_block"] if log_file else []
        output: str = ""
        for log in block_files:
            block_log_arg = ["--block-log", str(log)]
            with contextlib.suppress(BlockLogUtilError):
                output = self.__run_and_get_output("--get-block", *block_log_arg, *block_number_arg, *log_path_arg).replace("'", '"')
    ...
    def get_head_block_time(
        self, serialize: bool = False, serialize_format: TimeFormats | str = TimeFormats.DEFAULT_FORMAT, log_file: Path | None = None
    ) -> str | datetime:
        """
        ...
        Note: This method works correctly only for block logs with a length of at least 30 blocks.
        """
        head_block_num = self.get_head_block_number(log_file=log_file)
        head_block_timestamp = self.get_block(head_block_num, log_file=log_file).timestamp
        if serialize:
