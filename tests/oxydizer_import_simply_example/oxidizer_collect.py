...
for path in sys.path:
    ...
    if os.path.isdir(path):
        pass
for resource in oxidized_importer.find_resources_in_path('/usr/lib/python3.12'):
    if isinstance(resource, oxidized_importer.PythonExtensionModule):
        collector.add_filesystem_relative("oxidizer_simply_lib", resource)
    else:
...
with open("oxidized_resources_simply", "wb") as fh:
