...
async def test_witness_votes_user1_3(
    ...
    """https://gitlab.syncad.com/hive/clive/-/issues/127#3-test-cases-add-and-remove-votes-for-witnesses case 3.1."""
    # ARRANGE
    node, _, pilot = prepared_tui_on_dashboard_active
...
async def test_witness_votes_user1_4(
    ...
    # ACT
    await process_operation(pilot, "ADD_TO_CART", activated=True)
...
async def test_witness_votes_user1_5(
    ...
    for i in range(2):
        await focus_next(pilot)  # go to next witness on list
        expected_operations.append(await vote_for_witness(pilot, USER, generate_witness_name(4 + i)))
    ...
    await focus_next(pilot)  # go to first witness on list
    expected_operations.append(await vote_for_witness(pilot, USER, generate_witness_name(2)))

    # ACT
    await process_operation(pilot, "FINALIZE_TRANSACTION", activated=True)
...
async def test_witness_votes_user1_7(
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
    ...
    expected_operation = await vote_for_witness(pilot, USER)
    await process_operation(pilot, "FAST_BROADCAST", activated=True)
...
async def test_witness_votes_user1_9(
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
