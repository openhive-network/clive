...
from .witness_votes_utils import (
    USER3,
    WITNESSES_PER_PAGE,
    goto_first_witness,
    goto_governance_witness_list,
    unvote_witness,
    vote_witness,
)
...
@pytest.mark.parametrize("account", [USER3])
@pytest.mark.parametrize("operation_processing", TESTDATA)
async def test_witness_votes_user3_1(
    ...
    expected_operation = await vote_witness(pilot, USER3.name)
...
@pytest.mark.parametrize("account", [USER3])
@pytest.mark.parametrize("operation_processing", TESTDATA)
async def test_witness_votes_user3_2(
    ...
    await goto_governance_witness_list(pilot)
    await focus_next(pilot)  # go to first witness on list
    expected_operation = await unvote_witness(pilot, USER3.name)
    await process_operation(pilot, operation_processing, True)
...
@pytest.mark.parametrize("account", [USER3])
@pytest.mark.parametrize("operation_processing", TESTDATA)
async def test_witness_votes_user3_3(
    working_account: tt.Account,  # noqa: ARG001
    account: tt.Account,  # noqa: ARG001
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
    operation_processing: OperationProcessing,
    ...
    await focus_next(pilot)  # go to first witness on list
    expected_operations.append(await unvote_witness(pilot, USER3.name, "witness-02"))
    for _ in range(3):
        await focus_next(pilot)  # go to the next witness on list
    expected_operations.append(await vote_witness(pilot, USER3.name, "witness-04"))
    await focus_next(pilot)  # go to the next witness on list
    expected_operations.append(await vote_witness(pilot, USER3.name, "witness-05"))
...
@pytest.mark.parametrize("account", [USER3])
async def test_witness_votes_user3_4(
    working_account: tt.Account,  # noqa: ARG001
    account: tt.Account,  # noqa: ARG001
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
    ...
    await focus_next(pilot)  # go to first witness on list
    await goto_first_witness(pilot, "Vote")
    await vote_witness(pilot, USER3.name)  # this operation will be removed
    await focus_next(pilot)  # go to the next witness on list
    expected_operation = await vote_witness(pilot, USER3.name)
...
@pytest.mark.parametrize("account", [USER3])
async def test_witness_votes_user3_5(
    working_account: tt.Account,  # noqa: ARG001
    account: tt.Account,  # noqa: ARG001
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
    ...
    for i in range(VOTES):
        await focus_next(pilot)  # go to next witness on list
        await vote_witness(pilot, USER3.name, f"witness-0{4+i}")
    ...
    await focus_next(pilot)  # go to first witness on list
    expected_operation = await unvote_witness(pilot, USER3.name, "witness-02")
...
@pytest.mark.parametrize("account", [USER3])
async def test_witness_votes_user3_6(
    working_account: tt.Account,  # noqa: ARG001
    account: tt.Account,  # noqa: ARG001
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
    ...
    for witness in WITNESSES:
        if witness in ("witness-02", "witness-03"):
            await unvote_witness(pilot, USER3.name, witness)
        else:
            await vote_witness(pilot, USER3.name, witness)
        await focus_next(pilot)  # go to next witness on list


@pytest.mark.parametrize("account", [USER3])
async def test_witness_votes_user3_7(
    working_account: tt.Account,  # noqa: ARG001
    account: tt.Account,  # noqa: ARG001
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
...
@pytest.mark.parametrize("account", [USER3])
async def test_witness_votes_user3_8(
    working_account: tt.Account,  # noqa: ARG001
    account: tt.Account,  # noqa: ARG001
    prepared_tui_on_dashboard_active: tuple[tt.RawNode, tt.Wallet, ClivePilot],
) -> None:
