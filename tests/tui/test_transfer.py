...
async def test_transfers_finalize_cart(
    ...
    expected_operation0 = TransferOperation(from_=SENDER, to=RECEIVER, amount=ASSETS[0], memo=MEMOS[0])
    expected_operation1 = TransferOperation(from_=SENDER, to=RECEIVER, amount=ASSETS[1], memo=MEMOS[1])
    ...
    for i in range(2):
        ...
        await fill_transfer_data(pilot, RECEIVER, ASSETS[i], MEMOS[i])
        log_current_view(pilot.app, nodes=True)
