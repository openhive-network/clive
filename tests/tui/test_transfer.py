...
async def fill_transfer_data(
    ...
    await focus_next(pilot)  # focus on memo input
    if memo:
    ...
    await focus_next(pilot)  # focus back on `From` input
...
async def test_transfers(
    ...
    await focus_prev(
        pilot
    )  # Go to choose token. Alphanumeric bindings do not work in generic inputs, so we need to focus back
...
async def test_transfers_finalize_cart(prepared_tui_on_dashboard: tuple[tt.RawNode, tt.Wallet, ClivePilot]) -> None:
    ...
    for i in range(TRANSFERS_COUNT):
        ...
        await focus_prev(
            pilot
        )  # Go to finalize transaction button. Alphanumeric bindings do not work in generic inputs
    ...
    await press_and_wait_for_screen(
        pilot, CLIVE_PREDEFINED_BINDINGS.global_.transaction_summary.key, TransactionSummary
