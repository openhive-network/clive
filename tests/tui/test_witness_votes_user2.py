...
from clive_local_tools.testnet_block_log.constants import WATCHED_ACCOUNTS
from clive.__private.storage.accounts import Account as WatchedAccount
from clive.__private.storage.accounts import WorkingAccount
from clive.__private.ui.operations.cart import Cart, CartItem
from clive.__private.ui.operations.governance_operations.witness.witness import WitnessActionRow
from clive.__private.ui.operations.governance_operations.governance_operations import Governance
from clive.__private.ui.operations.governance_operations.witness.witness import Witness, WitnessNameLabel
...
from .witness_votes_utils import USER2, WITNESSES_PER_PAGE, goto_first_witness, goto_governance_witness_list, unvote_witness, vote_witness
...
@pytest.fixture()
def working_account() -> tt.Account:
    return USER2
...
async def test_witness_votes_user2_2(
    ...
    await goto_governance_witness_list(pilot)
    await focus_next(pilot)  # go to first witness on list
    expected_operation = await unvote_witness(pilot, USER)
    await process_operation(pilot, operation_processing, True)
...
async def test_witness_votes_user2_3(
    ...
    3. Test cases: Add and remove votes for witnesses.
        ...
        3.2. User 2 votes for two witnesses (witness_04 and witness_05) and removes vote for witness_02 and then broadcast the transaction (a/b/c).
    """
    # ARRANGE
    node, _, pilot = prepared_tui_on_dashboard_active
    ...
    await focus_next(pilot)  # go to first witness on list
    expected_operations.append(await unvote_witness(pilot, USER, "witness-02"))
    for _ in range(3):
        await focus_next(pilot)  # go to the next witness on list
    expected_operations.append(await vote_witness(pilot, USER, "witness-04"))
    await focus_next(pilot)  # go to the next witness on list
    expected_operations.append(await vote_witness(pilot, USER, "witness-05"))
...
async def test_witness_votes_user2_4(
    ...
    await focus_next(pilot)  # go to first witness on list
    await goto_first_witness(pilot, "Vote")
    await vote_witness(pilot, USER)  # this operation will be removed
    await focus_next(pilot)  # go to the next witness on list
    expected_operation = await vote_witness(pilot, USER)
...
async def test_witness_votes_user2_5(
    ...
    for i in range(2):
        await focus_next(pilot)  # go to next witness on list
        await vote_witness(pilot, USER, f"witness-0{4+i}")
    ...
    await focus_next(pilot)  # go to first witness on list
    expected_operation = await unvote_witness(pilot, USER, "witness-02")
    ...
    assert_operations_placed_in_blockchain(node, transaction_id, expected_operation)
    actions = pilot.app.screen.query(WitnessActionRow)
    assert len(actions) == 2, f"Expected 2 pending actions! Current number is: {len(actions)} action(s)."
...
async def test_witness_votes_user2_6(
    ...
    # ARRANGE
    node, _, pilot = prepared_tui_on_dashboard_active
    WITNESSES: Final[tuple[str]] = ("witness-02", "witness-03", "witness-01", "witness-04", "witness-05")
    ...
    for witness in WITNESSES:
        if witness in ("witness-02", "witness-03"):
            await unvote_witness(pilot, USER, witness)
        else:
            await vote_witness(pilot, USER, witness)
        await focus_next(pilot)  # go to next witness on list
...
async def test_witness_votes_user2_7(
    ...
    assert len(witnesses) == WITNESSES_PER_PAGE, f"Expected {WITNESSES_PER_PAGE} witnesses in view! Current number is: {len(witnesses)} witness(es)."
...
async def test_witness_votes_user2_8(
    ...
    # ASSERT
    witnesses: Witness = pilot.app.screen.query(Witness)
    assert len(witnesses) == WITNESSES_COUNT, f"Expected {WITNESSES_COUNT} witnesses in view! Current number is: {len(witnesses)} witness(es)."
