...
def create_witnesses(wallet: tt.Wallet) -> None:
    ...
    with wallet.in_single_transaction():
        for witness in WITNESSES:
            wallet.api.transfer("initminer", witness.name, tt.Asset.Test(10_000).as_serialized_nai(), "memo")
            wallet.api.transfer_to_vesting("initminer", witness.name, tt.Asset.Test(10_000).as_serialized_nai())
            wallet.api.transfer("initminer", witness.name, tt.Asset.Tbd(10_000).as_serialized_nai(), memo="memo")
    ...
    with wallet.in_single_transaction():
        for witness in WITNESSES:
            wallet.api.update_witness(
                ...
                witness.public_key,
                {
                    "account_creation_fee": tt.Asset.Test(3).as_serialized_nai(),
    ...
    with wallet.in_single_transaction():
        for i, (witness, proposal) in enumerate(zip(WITNESSES, PROPOSALS, strict=False)):
            ...
            wallet.api.create_proposal(
                ...
                tt.Asset.Tbd(daily_pay).as_serialized_nai(),
                permlink,
                f"{permlink}",
            )
...
def create_working_accounts(wallet: tt.Wallet) -> None:
    ...
    wallet.create_account(
        ...
        hives=WORKING_ACCOUNT_DATA.hives_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        vests=WORKING_ACCOUNT_DATA.vests.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        hbds=WORKING_ACCOUNT_DATA.hbds_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
    )
    wallet.create_account(
        ALT_WORKING_ACCOUNT1_DATA.account.name,
        hives=ALT_WORKING_ACCOUNT1_DATA.hives_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        vests=ALT_WORKING_ACCOUNT1_DATA.vests.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        hbds=ALT_WORKING_ACCOUNT1_DATA.hbds_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
    )
    wallet.create_account(
        ALT_WORKING_ACCOUNT2_DATA.account.name,
        hives=ALT_WORKING_ACCOUNT2_DATA.hives_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        vests=ALT_WORKING_ACCOUNT2_DATA.vests.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        hbds=ALT_WORKING_ACCOUNT2_DATA.hbds_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
    )
    ...
    for data in all_accounts:
        if data.hives_savings > 0:
            wallet.api.transfer_to_savings(
                ...
                data.account.name,
                data.hives_savings.as_serialized_nai(),
        ...
        if data.hbds_savings > 0:
            wallet.api.transfer_to_savings(
                CREATOR_ACCOUNT.name,
                data.account.name,
                data.hbds_savings.as_serialized_nai(),
        ...
        if data.hives_savings_withdrawal > 0:
            wallet.api.transfer_from_savings(
                ...
                0,
                CREATOR_ACCOUNT.name,
                data.hives_savings_withdrawal.as_serialized_nai(),
        ...
        if data.hbds_savings_withdrawal > 0:
            wallet.api.transfer_from_savings(
                ...
                0,
                CREATOR_ACCOUNT.name,
                data.hbds_savings_withdrawal.as_serialized_nai(),
...
def create_watched_accounts(wallet: tt.Wallet) -> None:
    ...
    for data in WATCHED_ACCOUNTS_DATA:
        wallet.create_account(
            data.account.name,
            hives=data.hives_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
            vests=data.vests.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
            hbds=data.hbds_liquid.as_serialized_nai(),  # type: ignore[arg-type]  # test-tools dooesn't convert to hf26
        )
