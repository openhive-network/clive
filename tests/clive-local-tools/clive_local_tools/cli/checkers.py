...
def in_range(ref: float, amount: float, tolerance: float) -> bool:
    """
    Checks if `amount` is equal to `ref` with error tolerance given as percent.
...
def assert_output_contains(expected_output: str, output: str) -> None:
...
def assert_effective_voting_power(context: CLITester | Result, asset: tt.Asset.HiveT) -> None:
    ...
    ), f"no entry for `{hive_power_type}` voting power with amount `{asset.as_legacy()}` in show hive-power output:\n{output}"
...
def assert_withdraw_routes(context: CLITester | Result, to: str, percent: int, auto_vest: bool = False) -> None:
    ...
    ), f"no withdraw route for `{to}` with percent `{expected_output}` and `{auto_vest=}`in show hive-power output:\n{output}"
...
def assert_no_pending_power_down(context: CLITester | Result) -> None:
    ...
    assert_output_contains(expected_output, output)
