...
async def prepare_persistent_storage_service() -> AsyncGenerator[PersistentStorageService, None]:
    async with await AsyncBeekeeper.factory(settings=safe_settings.beekeeper.settings_local_factory()) as beekeeper:
        ...
        wallet = await session.open_wallet(name=ALT_WORKING_ACCOUNT1_NAME)
        unlocked_wallet = await wallet.unlock(password=ALT_WORKING_ACCOUNT1_PASSWORD)
        encryption_wallet = await session.open_wallet(
            name=EncryptionService.get_encryption_wallet_name(ALT_WORKING_ACCOUNT1_NAME)
        )
