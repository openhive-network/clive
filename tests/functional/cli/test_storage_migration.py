...
async def test_unlock_old_profile(cli_tester_locked: CLITester) -> None:
    # ACT
    result = cli_tester_locked.unlock(profile_name=PROFILE_NAME, password_stdin=PROFILE_NAME * 2)
...
async def test_load_migrated_profile(cli_tester_locked: CLITester) -> None:
    # ARRANGE
    cli_tester_locked.unlock(profile_name=PROFILE_NAME, password_stdin=PROFILE_NAME * 2)
