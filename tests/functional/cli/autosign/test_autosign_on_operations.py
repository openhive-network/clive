...
async def test_autosign_with_multiple_keys_in_profile(
    node: tt.RawNode,
    cli_tester: CLITester,
) -> None:
    """Test autosigning succeeds when there are multiple keys available."""
    # ARRANGE
...
async def test_dry_run_of_autosigned_operation_on_profile_with_multiple_keys(
    ...
    *,
    ...
    """
    Test printing out dry run and transaction.

    Autosign should succeed even when there are multiple keys in the profile.
    """
    # ARRANGE
    cli_tester.configure_key_add(key=ADDITIONAL_KEY_VALUE, alias=ADDITIONAL_KEY_ALIAS_NAME)
