...
EXPECTED_MESSAGE: Final[str] = CLITransactionUnknownAccountError(UNKNOWN_ACCOUNT).message[:10]


@pytest.fixture
async def cli_tester_enabled_known_accounts(cli_tester: CLITester) -> CLITester:
...
async def test_enabled_known_accounts_delegate_vesting_shares_set(
    cli_tester_enabled_known_accounts: CLITester, amount: tt.Asset.VestT | tt.Asset.HiveT
) -> None:
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match=EXPECTED_MESSAGE):
        cli_tester_enabled_known_accounts.process_delegations_set(
            delegatee=UNKNOWN_ACCOUNT, amount=amount, sign=WORKING_ACCOUNT_KEY_ALIAS
        )


@pytest.mark.parametrize("amount", [tt.Asset.Vest(5000.000), tt.Asset.Hive(10.000)])
async def test_enabled_known_accounts_delegate_vesting_shares_remove(
    cli_tester: CLITester, amount: tt.Asset.VestT | tt.Asset.HiveT
) -> None:
    # ARRANGE
    cli_tester.configure_known_account_add(account_name=UNKNOWN_ACCOUNT)
    cli_tester.process_delegations_set(delegatee=UNKNOWN_ACCOUNT, amount=amount, sign=WORKING_ACCOUNT_KEY_ALIAS)
    cli_tester.configure_known_account_remove(account_name=UNKNOWN_ACCOUNT)

    # ACT & ASSERT
    cli_tester.process_delegations_remove(delegatee=UNKNOWN_ACCOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS)


async def test_enabled_known_accounts_set_withdraw_vesting_route_set(
...
async def test_enabled_known_accounts_transfer_from_savings(
...
@pytest.mark.parametrize("amount", [tt.Asset.Hbd(10.000), tt.Asset.Hive(10.000)])
async def test_enabled_known_accounts_transfer_from_savings_cancel(
    cli_tester: CLITester, amount: tt.Asset.HbdT | tt.Asset.HiveT
) -> None:
    # ARRANGE
    request_id: int = 13
    ...
    with pytest.raises(CLITestCommandError, match=EXPECTED_MESSAGE):
        cli_tester.process_savings_withdrawal_cancel(
            from_=UNKNOWN_ACCOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS, request_id=request_id
        )


@pytest.mark.parametrize("amount", [tt.Asset.Hbd(10.000), tt.Asset.Hive(10.000)])
async def test_enabled_known_accounts_transfer_to_savings(
    cli_tester: CLITester, amount: tt.Asset.HbdT | tt.Asset.HiveT
) -> None:
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match=EXPECTED_MESSAGE):
...
async def test_enabled_known_accounts_transfer_to_vesting(cli_tester: CLITester) -> None:
    # ARRANGE
