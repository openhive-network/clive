...
async def test_set_threshold_offline(testing_cli: TestingCli, authority: AuthorityType) -> None:
    ...
    testing_cli.process_update_authority(
        ...
        sign=str(WORKING_ACCOUNT_KEY_ALIAS),
        threshold=str(WEIGHT_THRESHOLD),
    ).add_account(account=WORKING_ACCOUNT.name, weight=str(WEIGHT)).fire()
...
async def test_add_account_offline(testing_cli: TestingCli, authority: AuthorityType) -> None:
    # ACT
    testing_cli.process_update_authority(
        authority, "--force-offline", password=WORKING_ACCOUNT.name, sign=WORKING_ACCOUNT_KEY_ALIAS
    ).add_account(account=OTHER_ACCOUNT.name, weight=str(WEIGHT)).fire()
...
async def test_modify_key_offline(testing_cli: TestingCli, authority: AuthorityType) -> None:
    ...
    testing_cli.process_update_authority(
        authority, "--force-offline", password=WORKING_ACCOUNT.name, sign=WORKING_ACCOUNT_KEY_ALIAS
    ).add_key(key=WORKING_ACCOUNT.public_key, weight=str(WEIGHT)).modify_key(
