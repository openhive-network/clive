...
async def test_negative_sign_option_multiple_times(cli_tester: CLITester, authority: AuthorityType) -> None:
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match=OptionAlreadySetError("sign").format_message()):
        cli_tester.process_update_authority(
...
async def test_negative_save_file_option_multiple_times(
    ...
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match=OptionAlreadySetError("save_file").format_message()):
