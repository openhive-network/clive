...
async def test_add_account(cli_with_runner: tuple[CliveTyper, CliRunner], authority: AuthorityType) -> None:
    # ARRANGE
...
@pytest.mark.parametrize("authority", get_args(AuthorityType))
async def test_remove_account(cli_with_runner: tuple[CliveTyper, CliRunner], authority: AuthorityType) -> None:
    # ARRANGE
    cli, runner = cli_with_runner
    other_account = WATCHED_ACCOUNTS[0]
    ...
    # ASSERT
    assert_is_not_authority(runner, cli, other_account.name, authority)


@pytest.mark.parametrize("authority", get_args(AuthorityType))
async def test_modify_account(cli_with_runner: tuple[CliveTyper, CliRunner], authority: AuthorityType) -> None:
    # ARRANGE
    cli, runner = cli_with_runner
    ...
    # ASSERT
    assert_is_authority(runner, cli, other_account.name, authority)
    assert_authority_weight(runner, cli, other_account.name, authority, modified_weight)
