...
@pytest.fixture
def transaction_path(tmp_path: Path) -> Callable[[str], Path]:
    def _create_transaction(receiver: str) -> Path:
        transaction_path = tmp_path / f"{receiver}_trx.json"
        ...
        with Path(transaction_path).open("w") as f:
...
async def test_loading_transaction(
    cli_tester: CLITester, receiver: str, transaction_path: Callable[[str], Path]
) -> None:
    # ARRANGE
    def perform_operation() -> None:
        trx_path = transaction_path(receiver)
        cli_tester.process_transaction(from_file=trx_path, sign=WORKING_ACCOUNT_KEY_ALIAS, broadcast=False)
