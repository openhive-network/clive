...
async def test_loading_transaction(cli_tester: CLITester, transaction_with_bad_account_path: Path) -> None:
    ...
    def perform_operation() -> None:
        cli_tester.process_transaction(
            from_file=transaction_with_bad_account_path, sign=WORKING_ACCOUNT_KEY_ALIAS, broadcast=False
        )
...
async def test_no_validation_of_removing_withdraw_routes_to_account_that_become_bad(
    ...
    # ARRANGE
    percent: int = 30
    cli_tester.process_withdraw_routes_set(to=TEMPORARY_BAD_ACCOUNT, percent=percent, sign=WORKING_ACCOUNT_KEY_ALIAS)

    def send_operation() -> None:
        cli_tester.process_withdraw_routes_remove(to=TEMPORARY_BAD_ACCOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS)
...
async def test_no_validation_of_removing_delegation_of_vesting_shares_to_account_that_become_bad(
    ...
    # ARRANGE
    cli_tester.process_delegations_set(delegatee=TEMPORARY_BAD_ACCOUNT, amount=AMOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS)

    def send_operation() -> None:
        cli_tester.process_delegations_remove(delegatee=TEMPORARY_BAD_ACCOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS)
...
async def test_no_validation_of_canceling_proxy_to_account_that_become_bad(
    ...
    # ARRANGE
    cli_tester.process_proxy_set(proxy=TEMPORARY_BAD_ACCOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS)
...
async def test_no_validation_of_removing_scheduled_transfer_to_account_that_become_bad(
    ...
    def send_operation() -> None:
        cli_tester.process_transfer_schedule_remove(to=TEMPORARY_BAD_ACCOUNT, sign=WORKING_ACCOUNT_KEY_ALIAS)
