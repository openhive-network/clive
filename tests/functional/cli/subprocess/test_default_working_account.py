...
async def test_negative_no_unlocked_profile(
    ...
    # ARRANGE
    expected_error = "Unhandled exception BeekeeperNotUnlockedError: There must be exactly one"
...
async def test_no_working_account(
    node: tt.RawNode,  # noqa: ARG001
...
async def test_negative_no_working_account(
    node: tt.RawNode,  # noqa: ARG001
    prepare_profile_without_working_account: Profile,  # noqa: ARG001
    cli_tester: CLITester,  # noqa: ARG001
) -> None:
...
async def test_custom_working_account(
    node: tt.RawNode,  # noqa: ARG001
    cli_tester: CLITester,  # noqa: ARG001
) -> None:
    # ARRANGE
...
async def test_custom_authority_in_custom_json_operation(
    node: tt.RawNode,
    prepare_wallet_extra_keys: None,  # noqa: ARG001
    cli_tester: CLITester,  # noqa: ARG001
) -> None:
    # ARRANGE
    ...
    operation = CustomJsonOperation(
        required_auths=[],
        required_posting_auths=[ALT_WORKING_ACCOUNT1_DATA.account.name],
        id_=custom_id,
        json_=custom_json,
    )
    command = [
        ...
        f"--authorize={ALT_WORKING_ACCOUNT1_DATA.account.name}",
    ]
