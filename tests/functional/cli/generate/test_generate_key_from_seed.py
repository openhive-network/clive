...
@pytest.mark.parametrize(
    ("seed", "account_name", "role", "public_key"),
    [
...
async def test_generate_key_from_seed_hardcoded(
    cli_tester_locked: CLITester, seed: str, account_name: str, role: str, public_key: str
) -> None:
    ...
    assert public_key == actual_public_key, (
        f"Actual public key `{actual_public_key}`)` doesn't match expected `{public_key}`."
    )
