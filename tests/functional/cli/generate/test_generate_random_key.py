...
async def test_default_key_pairs(cli_tester_without_remote_address: CLITester) -> None:
    """Check clive generate random-key command."""
    # ACT
    result = cli_tester_without_remote_address.generate_random_key()
...
async def test_custom_key_pairs(cli_tester_without_remote_address: CLITester) -> None:
    """Check clive generate random-key command with not default `--key-pairs` argument."""
    # ARRANGE
    key_pairs = 5

    # ACT
    result = cli_tester_without_remote_address.generate_random_key(key_pairs=key_pairs)
