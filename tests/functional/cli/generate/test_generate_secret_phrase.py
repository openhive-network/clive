...
async def test_generate_secret_phrase_length(cli_tester_without_remote_address: CLITester) -> None:
    """Check that clive generate secret-phrase command generates 16 worlds."""
    # ACT
    result = cli_tester_without_remote_address.generate_secret_phrase()
...
async def test_generate_secret_phrase_can_be_used_to_derive_keys(cli_tester_without_remote_address: CLITester) -> None:
    """Check that clive generate secret-phrase command can be input to command clive generate key-from-seed."""
    # ARRANGE
    result_secret_phrase = cli_tester_without_remote_address.generate_secret_phrase()
    seed = result_secret_phrase.stdout.strip()
    role = "active"

    # ACT
    result_key_from_seed = cli_tester_without_remote_address.generate_key_from_seed(
