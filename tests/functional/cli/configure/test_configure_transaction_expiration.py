...
async def test_negative_configure_transaction_expiration_below_min(cli_tester: CLITester, expiration: str) -> None:
    ...
    # ARRANGE
    td = shorthand_timedelta_to_timedelta(expiration)
    message = get_formatted_error_message(CLIInvalidTransactionExpirationError(InvalidTransactionExpirationError(td)))
...
async def test_negative_configure_transaction_expiration_invalid_format(cli_tester: CLITester) -> None:
    ...
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match="Invalid transaction expiration format"):
...
async def test_configure_transaction_expiration_unset(cli_tester: CLITester) -> None:
    """Check if unsetting transaction expiration restores the default."""
    # ARRANGE
    cli_tester.configure_transaction_expiration_set("1h")

    # ACT
    cli_tester.configure_transaction_expiration_unset()
...
async def test_configure_transaction_expiration_unset_when_already_default(cli_tester: CLITester) -> None:
    """Check if unsetting when already default still shows default."""
    # ACT
    cli_tester.configure_transaction_expiration_unset()
...
async def test_configure_transaction_expiration_show_default_annotation(cli_tester: CLITester) -> None:
    """Check that show profile displays '(default)' only when expiration is unset."""
