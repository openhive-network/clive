...
async def test_configure_tracked_account_add(
    cli_tester: CLITester, cli_profile_checker: ProfileAccountsChecker
) -> None:
    ...
    # ASSERT
    await cli_profile_checker.assert_in_tracked_accounts(account_names=[account_to_add])
    await cli_profile_checker.assert_in_known_accounts(account_names=[account_to_add])
...
async def test_configure_tracked_account_remove(
    cli_tester: CLITester, cli_profile_checker: ProfileAccountsChecker
) -> None:
    """Check clive configure tracked-account remove command."""
    # ARRANGE
    # ACT
    await cli_profile_checker.assert_in_tracked_accounts(account_names=[ACCOUNT_TO_REMOVE])
    cli_tester.configure_tracked_account_remove(account_name=ACCOUNT_TO_REMOVE)

    # ASSERT
    await cli_profile_checker.assert_not_in_tracked_accounts(account_names=[ACCOUNT_TO_REMOVE])
