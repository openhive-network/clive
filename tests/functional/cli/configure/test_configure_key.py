...
async def assert_key_exists(beekeeper: Beekeeper, private_key: PrivateKey, *, should_exists: bool) -> None:
    wallets_in_beekeeper = (await beekeeper.api.list_created_wallets()).wallets
    clive_wallet_details = next(
        (wallet for wallet in wallets_in_beekeeper if wallet.name == WORKING_ACCOUNT_NAME), None
    )

    assert clive_wallet_details, "Wallet should exists."

    if not clive_wallet_details.unlocked:
...
async def test_configure_key_add(
    world: World,
...
async def test_configure_key_remove(world: World, cli_tester: CLITester, *, from_beekeeper: bool) -> None:
