...
async def test_configure_key_add(beekeeper_unlocked: Beekeeper, cli_tester: CLITester) -> None:
    """Check clive configure key add command."""
    # ARRANGE
    pk = PrivateKey.create()

    await assert_key_exists(beekeeper_unlocked, pk, should_exists=False)

    # ACT
    cli_tester.configure_key_add(key=pk.value, alias="add_key")

    # ASSERT
    await assert_key_exists(beekeeper_unlocked, pk, should_exists=True)


@pytest.mark.parametrize("from_beekeeper", [True, False])
async def test_configure_key_remove(
    beekeeper_unlocked: Beekeeper, cli_tester: CLITester, *, from_beekeeper: bool
) -> None:
    ...
    # ASSERT
    await assert_key_exists(beekeeper_unlocked, pk, should_exists=not from_beekeeper)
