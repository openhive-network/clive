...
async def test_configure_working_account_switch(
    ...
    # ACT
    await cli_profile_checker.assert_working_account(working_account=WORKING_ACCOUNT_NAME)
    await cli_profile_checker.assert_in_tracked_accounts(account_names=[account_to_switch])

    cli_tester.configure_working_account_switch(account_name=account_to_switch)

    # ASSERT
    await cli_profile_checker.assert_working_account(working_account=account_to_switch)
    await cli_profile_checker.assert_in_tracked_accounts(account_names=[account_to_switch])
