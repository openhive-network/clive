...
async def test_decrypt_memo_from_block_log(
    ...
    # ASSERT
    expected_decrypted = ENCRYPTED_MEMO_CONTENT.removeprefix("#")
    assert expected_decrypted in result.stdout, (
        f"Decrypted content does not match expected content, expected {expected_decrypted}, got {result.stdout}"
    )
