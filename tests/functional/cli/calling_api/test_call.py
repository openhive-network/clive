...
def list_api_method_info() -> list[ApiMethodInfo]:
    ...
    for api_name in HIVED_API_NAMES:
        module = importlib.import_module(f"hiveio_api.{api_name}.{api_name}_description")
...
@pytest.mark.parametrize(
    ...
    [
        ("block_api", "get_block", '{"block_num": 10}'),
        ("block_api", "get_block", '{"block_num": "10"}'),
...
def test_negative_invalid_api_name(cli_tester: CLITester) -> None:
    ...
    expected_error = get_formatted_error_message(CLICallAPIModuleNotFoundError(invalid_api_name))
