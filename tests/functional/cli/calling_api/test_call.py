...
from clive.__private.cli.commands.call import (
    CLIApiPackageNotFoundError,
    CLIMethodNotFoundError,
    CLINoNodeAddressError,
    CLIParamsDecodeError,
    CLIParamsFormatError,
)
...
@pytest.mark.parametrize(
    ("cli_tester_variant", "original_message"),
    [
        ("locked", CLINoProfileUnlockedError.MESSAGE),
        ("without remote address", CLIBeekeeperRemoteAddressIsNotSetError.MESSAGE),
        ("without session token", CLIBeekeeperSessionTokenNotSetError.MESSAGE),
    ],
    indirect=["cli_tester_variant"],
)
def test_negative_call_locked_without_node_address(cli_tester_variant: CLITester, original_message: str) -> None:
    # ARRANGE
    api_name = "database_api"
    method_name = "get_dynamic_global_properties"
...
def test_negative_invalid_api_name(cli_tester: CLITester) -> None:
    ...
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match=CLIApiPackageNotFoundError.get_message(invalid_api_name)):
...
def test_negative_invalid_method_name(cli_tester: CLITester) -> None:
    ...
    with pytest.raises(CLITestCommandError, match=CLIMethodNotFoundError.get_message(invalid_method_name, api_name)):
...
def test_negative_invalid_params_decode(cli_tester: CLITester) -> None:
    ...
    # ACT & ASSERT
    with pytest.raises(CLITestCommandError, match=CLIParamsDecodeError.get_message(raw_params)):
...
def test_negative_invalid_params_format(cli_tester: CLITester) -> None:
    ...
    # ARRANGE
    raw_params = '""'
    pattern = re.escape(CLIParamsFormatError.get_message(raw_params))
