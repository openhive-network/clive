...
async def test_negative_autosign_multi_account_transaction_with_insufficient_keys(
    ...
    # ACT & ASSERT - transaction signed with alice's key only, node rejects due to missing bob's authority
    with pytest.raises(CLITestCommandError, match="Missing active authority"):
