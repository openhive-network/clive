...
async def test_show_rc_section_headers(cli_tester: CLITester) -> None:
    # ACT
    result = cli_tester.show_rc()

    # ASSERT
    assert_output_contains("Resource Credits balance", result)
    assert_output_contains("RC Manabar", result)
...
async def test_show_rc_with_delegation_verifies_section_and_amount(cli_tester: CLITester) -> None:
    # ARRANGE
    cli_tester.process_rc_delegations_set(
        delegatee=DELEGATEE_ACCOUNT_NAME, amount=RC_DELEGATION_AMOUNT, sign_with=WORKING_ACCOUNT_KEY_ALIAS
    )
    ...
    # ASSERT
    assert_output_contains("Current outgoing RC delegations", result)
    assert_output_contains("RC Amount", result)
    assert_output_contains("VESTS", result)
