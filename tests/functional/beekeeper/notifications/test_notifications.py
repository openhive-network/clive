...
LOG_FILE_PATH = Path(Beekeeper().get_wallet_dir()).parent / "logs" / "debug" / "latest.log"
...
@pytest.mark.parametrize("current_status", ["signals_attached", "interrupted", "beekeeper is starting"])
async def test_notification_hived_status(current_status: str) -> None:
    ...
    # Warning!
    # I gave current_status!="signals_attached" because of issue:
    # https://gitlab.syncad.com/hive/hive/-/issues/624
    # When it will be fixed this assertion should info us about it.
    assert notification_msg_found == (
        current_status != "signals_attached"
    ), f"There should be `{current_status}` notification."


async def test_notification_opening_beekeeper_failed(beekeeper: Beekeeper) -> None:  # noqa: ARG001
    ...
    # ACT
    # Because of https://gitlab.syncad.com/hive/hive/-/issues/624 we got assert here...
    bk = Beekeeper()
    ...
    # When this issue will be closed, there should be different notification
    notification_msg_found = checkers.check_for_pattern_in_file(
        Path(Beekeeper().get_wallet_dir()).parent / "logs" / "debug" / "latest.log", notification_msg
    )
...
async def test_notification_attempt_of_closing_all_wallets() -> None:
    ...
    # ARRANGE
    beekeeper = await Beekeeper().launch(unlock_timeout=1)
