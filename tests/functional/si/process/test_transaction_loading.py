...
async def test_broadcast_unsigned_transaction_from_file_with_signing(
    clive_si: UnlockedCliveSi,
    node: tt.RawNode,
    tmp_path: Path,
) -> None:
    """Test broadcasting an unsigned transaction loaded from file by signing it."""
    file_path = tmp_path / "unsigned_transfer.json"
    balance_before = node.api.wallet_bridge.get_account(WORKING_ACCOUNT_NAME).balance  # type: ignore[union-attr]

    # Create and save unsigned transaction
    await clive_si.process.transfer(
        from_account=WORKING_ACCOUNT_NAME,
        to_account=RECEIVER,
        amount=AMOUNT,
    ...
    # Load unsigned transaction and sign during broadcast
    await (
        clive_si.process.transaction(from_file=file_path, force_unsign=True)
        .sign_with(key=WORKING_ACCOUNT_KEY_ALIAS)
        .broadcast()
    )
...
async def test_broadcast_unsigned_transaction_from_file_with_autosign(
    clive_si: UnlockedCliveSi,
    node: tt.RawNode,
    tmp_path: Path,
) -> None:
    """Test broadcasting an unsigned transaction loaded from file using autosign."""
    file_path = tmp_path / "unsigned_transfer_autosign.json"
    balance_before = node.api.wallet_bridge.get_account(WORKING_ACCOUNT_NAME).balance  # type: ignore[union-attr]
    ...
    # Load unsigned transaction and autosign during broadcast
    await clive_si.process.transaction(from_file=file_path, force_unsign=True).autosign().broadcast()
...
async def test_broadcast_transaction_from_file_with_signature_override(
    ...
    balance_before = node_two_keys_profile.api.wallet_bridge.get_account(WORKING_ACCOUNT_NAME).balance  # type: ignore[union-attr]
...
async def test_broadcast_transaction_from_object_with_signature_override(
    clive_si_with_two_keys_profile: UnlockedCliveSi,
    node_two_keys_profile: tt.RawNode,
) -> None:
