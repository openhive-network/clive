...
from clive.__private.si.base import SIWorld, UnlockedCliveSi
from clive.__private.si.data_classes import Accounts, AuthorityInfo, Balances, Witness
from clive.si import CliveSi
...
async def si_world(
...
) -> AsyncGenerator[SIWorld]:
    """Create SIWorld that uses the test beekeeper.
    ...
    si_world.beekeeper_manager.settings.http_endpoint = (
        world_with_remote_beekeeper.beekeeper_manager.beekeeper.settings.http_endpoint
    )
    si_world.beekeeper_manager.settings.use_existing_session = (
...
class TestShowBalances:
    async def test_show_balances_returns_balances_object(
        ...
        si = UnlockedCliveSi()
        si._world = si_world
        si._is_setup_called = True

        balances = await si.show.balances("alice")
...
class TestShowAccounts:
    async def test_show_accounts_returns_accounts_object(self, si_world: SIWorld) -> None:
        """Test that show.accounts() returns Accounts dataclass."""
        si = UnlockedCliveSi()
        si._world = si_world
        si._is_setup_called = True

        accounts = await si.show.accounts()
...
class TestShowWitnesses:
    async def test_show_witnesses_returns_list_of_witnesses(
        self,
        si_world: SIWorld,
        node: None,  # noqa: ARG002
    ) -> None:
        """Test that show.witnesses() returns list of Witness objects."""
        si = UnlockedCliveSi()
        si._world = si_world
        si._is_setup_called = True

        witnesses = await si.show.witnesses("alice", page_size=10, page_no=0)

        assert isinstance(witnesses, list)
        if len(witnesses) > 0:
            assert isinstance(witnesses[0], Witness)
            assert hasattr(witnesses[0], "witness_name")
            assert hasattr(witnesses[0], "votes")
            assert hasattr(witnesses[0], "voted")
...
class TestShowAuthority:
    async def test_show_owner_authority_returns_authority_info(
        ...
        authority = await si.show.owner_authority("alice")
    ...
    async def test_show_active_authority_returns_authority_info(
        ...
        authority = await si.show.active_authority("alice")
    ...
    async def test_show_posting_authority_returns_authority_info(
        ...
        authority = await si.show.posting_authority("alice")
