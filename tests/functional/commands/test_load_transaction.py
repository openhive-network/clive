...
VALID_TRANSACTION: Final[Transaction] = Transaction(
    operations=[HF26RepresentationTransferOperation(value=TransferOperation(from_="alice", to="bob", amount=Asset.hbd(1), memo="test"))],
    ref_block_num=1,
    ref_block_prefix=2,
...
async def test_loading_valid_transaction_file(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    ...
    if mode == "json":
        file_path.write_text(encoder.encode(expected_transaction).decode())
    else:
