...
async def test_withdraw_valid(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    # ARRANGE
...
@pytest.mark.parametrize("mode", ["json", "bin"])
async def test_withdraw_to_other_account(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    # ARRANGE
    expected_transaction = VALID_TRANSACTION
    file_name = f"transaction.{mode}"
    file_path = tmp_path / file_name
...
@pytest.mark.parametrize("mode", ["json", "bin"])
async def test_withdraw_invalid(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    # ARRANGE
    expected_transaction = VALID_TRANSACTION
    file_name = f"transaction.{mode}"
    file_path = tmp_path / file_name
