...
async def test_show_withdrawals(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    # ARRANGE
...
@pytest.mark.parametrize("mode", ["json", "bin"])
async def test_show_withdrawals_no_pending(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    # ARRANGE
    expected_transaction = VALID_TRANSACTION
    file_name = f"transaction.{mode}"
    file_path = tmp_path / file_name
...
@pytest.mark.parametrize("mode", ["json", "bin"])
async def test_show_withdrawals_no_account(tmp_path: Path, mode: Literal["json", "bin"]) -> None:
    # ARRANGE
    expected_transaction = VALID_TRANSACTION
    file_name = f"transaction.{mode}"
    file_path = tmp_path / file_name
