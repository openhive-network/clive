...
async def test_unlock(world: clive.World, prepare_wallet: WalletInfo) -> None:
    # ARRANGE
    await world.beekeeper.api.lock_all()

    # ACT
    await world.commands.unlock(password=prepare_wallet.password)
...
async def test_unlock_again(world: clive.World, prepare_wallet: WalletInfo) -> None:
    # ARRANGE & ACT
    assert world.app_state.is_unlocked
    await world.commands.lock()
    await world.commands.unlock(password=prepare_wallet.password)
...
async def test_lock_after_given_time(world: clive.World, prepare_wallet: WalletInfo) -> None:
    # ARRANGE
    time_to_sleep: Final[timedelta] = timedelta(seconds=2)
    await world.commands.lock()

    # ACT
    await world.commands.unlock(password=prepare_wallet.password, time=time_to_sleep)
    assert world.app_state.is_unlocked
