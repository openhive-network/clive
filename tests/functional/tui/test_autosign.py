...
async def test_autosign_tui_with_three_wallet_keys_two_in_authority(
    ...
    await focus_next(pilot)  # → exit MemoInput (Autosign is preselected in TransactionSummary)
...
async def test_negative_autosign_tui_with_insufficient_authority(
    ...
    from clive.exceptions import TransactionNotSignedError  # noqa: PLC0415

    expected_message = TransactionNotSignedError.MESSAGE
    found = await extract_message_from_notification(
        pilot, lambda message: message if expected_message in message else ""
    )
    assert found
...
async def test_autosign_tui_save_transaction_to_file(
    ...
    await focus_next(pilot)  # FileNameInput → BinaryCheckbox
    await focus_next(pilot)  # BinaryCheckbox → SignedCheckbox
    await pilot.press("space")  # check SignedCheckbox (transaction is unsigned by default)
    await focus_prev(pilot)  # SignedCheckbox → BinaryCheckbox
    await focus_prev(pilot)  # BinaryCheckbox → FileNameInput
...
async def test_negative_autosign_tui_save_transaction_to_file_with_insufficient_authority(
    ...
    """Test that save-to-file with autosign saves unsigned transaction when wallet keys don't match authority."""
    pilot = prepared_tui_autosign_insufficient
    ...
    # ASSERT - file saved but unsigned (autosign had no matching keys)
    assert save_path.exists(), "Transaction file should have been saved"
