...
SIGNING_MAX_WORKERS: Final[int] = 3
BROADCASTING_MAX_WORKERS: Final[int] = 4
...
async def test_prepare_second_stage_of_block_log(
    ...
    operations = prepare_operations_for_transactions(
        iterations=1, ops_in_one_element=1, elements_number_for_iteration=12000
    )
    ...
    async with await Beekeeper().launch() as beekeeper:
        ...
        with ProcessPoolExecutor(max_workers=SIGNING_MAX_WORKERS) as executor:
            ...
            for iteration, chunk in enumerate(chunks):
                futures.append(executor.submit(send_by_curl,
...
def create_and_sign_transaction(ops, gdpo, node_config, num, url, token, public_key):
    sig_digest_pack = {
        "jsonrpc": "2.0",
