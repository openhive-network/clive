...
from generate_block_log_with_varied_signature_types import WITNESSES

ALTERNATE_CHAIN_JSON_FILENAME = "alternate-chain-spec.json"

@pytest.fixture()
def replayed_node(request: pytest.FixtureRequest) -> tt.InitNode:
    block_log_directory = Path(__file__).parent / request.param[0]
    block_log = tt.BlockLog(Path(__file__).parent / request.param[0] / "block_log")
