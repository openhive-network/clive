...
def set_vest_price_by_alternate_chain_spec(file_path: Path) -> None:
...
def create_witnesses(wallet: tt.Wallet) -> None:
    ...
    with wallet.in_single_transaction():
        for name in WITNESSES:
            wallet.api.transfer("initminer", name, tt.Asset.Test(randint(10_000, 20_000)).as_nai(), "memo")
            wallet.api.transfer_to_vesting("initminer", name, tt.Asset.Test(randint(10_000, 20_000)).as_nai())
            wallet.api.transfer("initminer", name, tt.Asset.Tbd(randint(10_000, 20_000)).as_nai(), memo="memo")
...
def create_working_account(wallet: tt.Wallet) -> None:
    ...
    wallet.create_account(
        ...
        hives=tt.Asset.Test(100_000).as_nai(),
        vests=tt.Asset.Test(100_000).as_nai(),
        hbds=tt.Asset.Tbd(100_000).as_nai(),
    )
...
def create_watched_accounts(wallet: tt.Wallet) -> None:
    ...
    for account in WATCHED_ACCOUNTS:
        wallet.create_account(
            account.name,
            hives=tt.Asset.Test(randint(1_000, 5_000)).as_nai(),
            vests=tt.Asset.Test(randint(1_000, 5_000)).as_nai(),
            hbds=tt.Asset.Tbd(randint(1_000, 5_000)).as_nai(),
        )
...
def main() -> None:
    ...
    node = tt.InitNode()
    configure(node)
    set_vest_price_by_alternate_chain_spec(directory / "alternate-chain-spec.json")
