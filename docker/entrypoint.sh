...
PIPELINE=""
WORKING_DIRECTORY="/clive"
export CLIVE_SELECT_FILE_ROOT_PATH="/clive/.clive/mapped_host_directory"
...
launch_cli() {
  local activate_beekeeper_script_path="${WORKING_DIRECTORY}/scripts/activate_beekeeper.sh"
...
parse_arguments() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --exec)
        shift
        FILE_TO_EXECUTE="${WORKING_DIRECTORY}/$1"
...
check_if_clive_directory_exists() {
  if [[ ! -d ${WORKING_DIRECTORY} ]]; then
    echo "Error: ${WORKING_DIRECTORY} directory does not exist."
    exit 1
  fi
}
