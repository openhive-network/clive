...
from clive.exceptions import KeyBindingError
...
def get_bind_from_config(name: str) -> Keys:
    ...
    if not binding:
        raise KeyBindingError(f"Key binding `{name}` was not found in the settings.toml file")

    try:
        return Keys(binding)
    except ValueError as exception:
        raise KeyBindingError(f"Invalid key binding: {binding}.") from exception
