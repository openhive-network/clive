...
from clive.__private.storage.migrations import current, v1, v2, v3
...
AllProfileStorageModels: TypeAlias = v1.ProfileStorageModel | v2.ProfileStorageModel | v3.ProfileStorageModel
...
def upgrade_storage_model(model: AllProfileStorageModels) -> current.ProfileStorageModel:
    ...
    if isinstance(model, v2.ProfileStorageModel):
        model = v3.ProfileStorageModel(
            name=model.name,
            working_account=model.working_account,
            tracked_accounts=model.tracked_accounts,
            known_accounts=model.known_accounts,
...
def parse_and_upgrade_storage_model(content: str, version: str) -> current.ProfileStorageModel:
    ...
    match version:
        ...
        case "v3":
            model = v3.ProfileStorageModel.parse_raw(content)
        case _:
