...
class StoredVersionNotKnownError(CliveError):
    def __init__(self, file: Path) -> None:
        super().__init__(f"Failed to read stored revision from file: {file}")
...
class PersistentStorageService:
    ...
    async def _find_profile_storage_model_by_name(self, profile_name: str) -> ProfileStorageModel:
        ...
        for (n, v), path in filepaths.items():
            if n == profile_name and (selected_path is None or v > self._calculate_version(selected_path)):
                selected_path = path
