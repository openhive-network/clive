...
class InvalidPasswordLengthError(Exception):
    ...
    def __init__(self, min_length: int, max_length: int, current_length: int) -> None:
        super().__init__(f"Invalid password length: Current: {current_length}. Must be between {min_length} and {max_length} characters.")
...
class InvalidNumberOfKeyPairsError(Exception):
    ...
    def __init__(self, number_of_key_pairs: int) -> None:
        super().__init__(f"Invalid number of key pairs: {number_of_key_pairs}. Number of key pairs must be greater than 0.")
...
class InvalidAuthorityLevelError(Exception):
    ...
    def __init__(self, authority_level: str, valid_levels: list[str]) -> None:
        ...
        super().__init__(f"Invalid authority level: '{authority_level}'. Valid authority levels are: '{valid_levels_str}'.")
