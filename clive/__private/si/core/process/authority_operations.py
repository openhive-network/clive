"""Authority modification operations for ProcessAuthority."""
...
def add_key(auth: Authority, key: str, weight: int) -> Authority:
    ...
    if any(key == k for k, _ in auth.key_auths):
...
def add_account(auth: Authority, account: str, weight: int) -> Authority:
    ...
    if any(account == acc for acc, _ in auth.account_auths):
...
def remove_key(auth: Authority, key: str) -> Authority:
    ...
    # Check if key exists
    if not any(key == k for k, _ in auth.key_auths):
...
def remove_account(auth: Authority, account: str) -> Authority:
    ...
    if not any(account == acc for acc, _ in auth.account_auths):
