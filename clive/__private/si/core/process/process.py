...
class ProcessTransaction(ProcessCommandBase):
    ...
    async def validate(self) -> None:
        LoadTransactionFromFileFromObjectValidator().validate(
            from_file_argument=self.from_file,
            from_object_argument=self.from_object,
        )
        AlreadySignedModeValidator().validate(
            use_autosign_argument=self._autosign,
            already_signed_mode_argument=self.already_signed_mode,
        )
...
class ProcessAuthority(ProcessCommandBase):
    """Handles authority update operations by managing callbacks that modify authority structures."""
