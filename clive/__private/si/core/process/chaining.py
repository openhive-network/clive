...
class BaseChain(_FinalizingBase):
    ...
    def __init__(self, world: World, operation_builders: list[ProcessCommandBase] | None = None) -> None:
        self._previous_operation_builders = operation_builders or []
        self._operation_builder: ProcessCommandBase | None = None
        super().__init__(world, operation_builders=[])  # Will be provided via property
        self.main_command_type: str | None = None

    def _create_processor(self) -> ProcessCommandBase:  # type: ignore[empty-body]
        """Create the appropriate operation builder for this chain."""

    @property
    def _operation_builders(self) -> list[ProcessCommandBase]:
        """Get all operation builders including existing operations and current operation builder."""
        if self._operation_builder is None:
            return self._previous_operation_builders
        return [*self._previous_operation_builders, self._operation_builder]

    @_operation_builders.setter
    def _operation_builders(self, value: list[ProcessCommandBase]) -> None:
    ...
    def process(self) -> ProcessInterfaceBase:
        ...
        return ProcessInterfaceChaining(world=self.world, operation_builders=self._operation_builders)
    ...
    def sign_with(self, key: str) -> SignWithChain:
        ...
        return SignWithChain(self.world, self._operation_builders, sign_keys=[key])

    def autosign(self) -> AutoSignChain:
        """Configure transaction to be auto-signed. Does NOT allow chaining with sign_with()."""
        return AutoSignChain(self.world, self._operation_builders)
