...
class CLICallAPIModuleNotFoundError(CLIPrettyError):
    """
    Raise when api module for requested api is not found in installed hiveio-api package.
    ...
    def __init__(self, api_name: str) -> None:
        message = f"Module for api `{api_name}` not found in hiveio-api package."
...
class CLICallAPIClientModuleNotFoundError(CLIPrettyError):
    ...
    def __init__(self, api_client_module_name: str, api_name: str) -> None:
        message = (
            ...
            f"This might be a problem with installed hiveio-api package."
        )
        super().__init__(message, errno.EINVAL)
...
class _CallAPICommon(ExternalCLICommand, ABC):
    ...
    def build_extended_api_collection(self) -> type:
        ...
        class RequestedApiCollection:
            _API_MAP: ClassVar[dict[str, type]] = {api_name: api_class}
