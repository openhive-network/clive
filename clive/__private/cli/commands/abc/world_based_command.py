...
from clive.__private.cli.exceptions import (
    CLIEitherBeekeepersSessionTokenNotSetError,
...
class WorldBasedCommand(ContextualCLICommand[World], BeekeeperCommon, ABC):
    """A command that requires a world, either a password or session token are required to decrypt profile."""
    ...
    def _validate_session_token_set(self) -> None:
        if not self._credentials_provided():
            raise CLIEitherBeekeepersSessionTokenNotSetError

    def _credentials_provided(self) -> bool:
        return self.is_session_token_set()
