...
class ShowPendingPowerUps(WorldBasedCommand):
    ...
    async def _run(self) -> None:
        ...
        delayed_votes_table.add_column("Start time", justify="right", style="green", no_wrap=True)
        delayed_votes_table.add_column("Effective time", justify="right", style="green", no_wrap=True)
        ...
        for entry in delayed_votes:
            wax_json = iwax.vests(entry.val)
            votes_vests = Asset.from_wax_json(wax_json)
            if not isinstance(votes_vests, Asset.Vests):
                raise InvalidAssetError(votes_vests, Asset.Vests)
            delayed_votes_table.add_row(
                humanize_datetime(entry.time),
                humanize_datetime(entry.time + delayed_voting_interval),
                humanize_asset(vests_to_hive(votes_vests, gdpo)),
                humanize_asset(votes_vests),
            )
