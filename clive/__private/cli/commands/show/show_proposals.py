...
class ShowProposals(WorldBasedCommand):
    ...
    async def _run(self) -> None:
        ...
        table = Table(title=f"Proposals and votes of `{proxy if proxy else self.account_name}` account")

        table.add_column("voted", justify="left", style="cyan", overflow="fold")
        table.add_column("title", justify="right", style="green", overflow="fold", min_width=40)
        table.add_column("proposal id", justify="right", style="green", overflow="fold")
        ...
        table.add_column("start date", justify="right", style="green", overflow="fold")
        table.add_column("end date", justify="right", style="green", overflow="fold")
