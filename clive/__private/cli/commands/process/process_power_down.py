...
class ProcessPowerDown(OperationCommand):
    account_name: str
    hive_or_vests: Asset.VotingT = Asset.vests(0)
    ...
    async def _create_operation(self) -> WithdrawVestingOperation:
        ...
        if isinstance(self.hive_or_vests, Asset.Hive):
            gdpo = await self.world.node.api.database_api.get_dynamic_global_properties()
            vesting_shares = hive_to_vests(self.hive_or_vests, gdpo)
        else:
            vesting_shares = self.hive_or_vests
