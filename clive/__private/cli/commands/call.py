...
from clive.__private.models.schemas import JsonString
...
def get_api_description_module_path(api_name: str) -> str:
    return f"{api_name}.{api_name}_description"
...
class CLINoNodeAddressError(CLIPrettyError):
    ...
    Args:
        original_message: Message of original exception.

    Attributes:
        MESSAGE_ADD: A message added to original exception about node_address option.
    """

    MESSAGE_ADD: Final[str] = (
    ...
    def __init__(self, original_message: str) -> None:
        super().__init__(self.get_message(original_message), errno.EINVAL)

    @classmethod
    def get_message(cls, original_message: str) -> str:
        return f"{original_message} {cls.MESSAGE_ADD}"
...
class CLIApiPackageNotFoundError(CLIPrettyError):
    ...
    def __init__(self, api_name: str) -> None:
        super().__init__(self.get_message(api_name), errno.EINVAL)

    @classmethod
    def get_message(cls, api_name: str) -> str:
        return f"Package for api `{api_name}` not found."
...
class CLIApiClientModuleNotFoundError(CLIPrettyError):
    ...
    def __init__(self, api_client_module_name: str, api_name: str) -> None:
        super().__init__(self.get_message(api_client_module_name, api_name), errno.EINVAL)

    @classmethod
    def get_message(cls, api_client_module_name: str, api_name: str) -> str:
        return (
...
class CLIApiClassDefinitionNotFoundError(CLIPrettyError):
    ...
    def __init__(self, api_class_name: str, api_name: str) -> None:
        super().__init__(self.get_message(api_class_name, api_name), errno.EINVAL)

    @classmethod
    def get_message(cls, api_class_name: str, api_name: str) -> str:
        return (
...
class CLIMethodNotFoundError(CLIPrettyError):
    ...
    def __init__(self, method_name: str, api_name: str) -> None:
        super().__init__(self.get_message(method_name, api_name), errno.EINVAL)

    @classmethod
    def get_message(cls, method_name: str, api_name: str) -> str:
        return (
            f"No method `{method_name}` found in api `{api_name}`. This might be a problem with installed `{api_name}`."
        )


class CLIParamsDecodeError(CLIPrettyError):
    """
    ...
    def __init__(self, raw_params: str) -> None:
        super().__init__(self.get_message(raw_params), errno.EINVAL)

    @classmethod
    def get_message(cls, raw_params: str) -> str:
        return f"Could not parse parameters. Parameters must be a json string. Received `{raw_params}`"


class CLIParamsFormatError(CLIPrettyError):
    """
    Raise when passed parameters are not dict or list.
...
class Call(WorldBasedCommand):
    ...
    async def _run(self) -> None:
        collection_type = self._build_excented_api_collection()
    ...
    def _validate_api_call_params(self) -> None:
        try:
            ...
            raise CLIParamsDecodeError(self.params) from err
        if not isinstance(self._parsed_params, (list, dict)):
            raise CLIParamsFormatError(self.params)

    def _build_excented_api_collection(self) -> type:
