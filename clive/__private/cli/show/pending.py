...
@pending.command(name="power-downs", common_options=[WorldWithoutBeekeeperCommonOptions])
async def show_pending_power_downs(
    ctx: typer.Context,  # noqa: ARG001
    account_name: str = options.account_name_option,
) -> None:
    """Power down takes place every week for 13 weeks after power down operation."""
    from clive.__private.cli.commands.show.show_pending_power_downs import ShowPendingPowerDowns

    common = WorldWithoutBeekeeperCommonOptions.get_instance()
    await ShowPendingPowerDowns(**common.as_dict(), account_name=account_name).run()
