...
from clive.__private.cli.common.parsers import decimal_price, hive_datetime, liquid_asset
...
Examples:
  ...
  clive process order create --amount-to-sell "100.000 HIVE" --price 0.250
...
async def process_order_create(  # noqa: PLR0913
    ...
    price_value: str | None = typer.Option(
        None,
        "--price",
        parser=decimal_price,
        help=(
            "Price of 1 HIVE in HBD (e.g., 0.0725). "
    ...
    expiration: str = typer.Option(  # actually datetime | timedelta | None, but Typer doesn't support Union types
        None,
        "--expiration",
        parser=hive_datetime,
        help=stylized_help(
            "When the order expires (max 28 days)."
            " Formats: absolute (2025-12-31, 2025-12-31T14:30:00) or relative (+14d, +2w).",
    ...
    await ProcessOrderCreate(
        ...
        order_id=order_id,
        expiration=cast("datetime | timedelta | None", expiration),
