...
async def process_power_down_start(
    ...
    hive_or_vests: Asset.VotingT = options.hive_or_vests_option,
) -> None:
    ...
    operation = ProcessPowerDown(
        **common.as_dict(), account_name=account_name, hive_or_vests=hive_or_vests, only_new_power_down=True
    )
    await operation.run()
...
async def process_power_down_restart(
    ...
    common = OperationCommonOptions.get_instance()
    operation = ProcessPowerDown(**common.as_dict(), account_name=account_name, hive_or_vests=hive_or_vests)
    await operation.run()
...
async def process_power_down_cancel(
    ...
    common = OperationCommonOptions.get_instance()
    operation = ProcessPowerDown(**common.as_dict(), account_name=account_name)
    await operation.run()
...
async def process_delegations_set(
    ...
    operation = ProcessDelegations(
        **common.as_dict(), delegator=account_name, delegatee=delegatee, hive_or_vests=hive_or_vests
    )
    await operation.run()
