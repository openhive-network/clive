...
class SafeSettings:
    ...
    class _Beekeeper(_Namespace):
        ...
        def settings_factory(
            self, working_directory: Path | None = None, remote_endpoint: HttpUrl | None = None
        ) -> Settings:
            from beekeepy import Settings

            return Settings(
                working_directory=(working_directory or self._get_beekeeper_wallet_directory()),
                http_endpoint=remote_endpoint,
                use_existing_session=self.session_token,
                timeout=timedelta(seconds=self.communication_total_timeout_secs),
            )
