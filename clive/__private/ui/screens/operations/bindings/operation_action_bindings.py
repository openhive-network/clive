...
class OperationActionBindings(CliveWidget, AbstractClassMessagePump):
    ...
    def _add_to_cart(self, operations: list[OperationUnion]) -> None:
    ...
    def _are_given_operations_already_in_profile_transaction(self, operations: list[OperationUnion]) -> bool:
        return any(operation in self.profile.transaction for operation in operations)
    ...
    async def _handle_add_to_cart_request(self) -> None:
        ...
        if (
            not self.ALLOW_THE_SAME_OPERATION_IN_CART_MULTIPLE_TIMES
            and self._are_given_operations_already_in_profile_transaction(operations_to_add)
        ):
