...
class TransactionSummary(BaseScreen):
    ...
    def __init__(self, transaction: Transaction | None) -> None:
        ...
        self._key_container = KeyContainer(transaction)
    ...
    def watch_transaction(self, _: Transaction | None) -> None:
        self._transaction_metadata_container.transaction = self.transaction
        self._button_container.transaction = self.transaction
        self._key_container.transaction = self.transaction
        self._key_container.display = bool(self.transaction)
    ...
    def _get_key_to_sign(self) -> PublicKey:
        return self._key_container.selected_key
