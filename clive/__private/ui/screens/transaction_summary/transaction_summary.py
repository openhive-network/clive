...
class TransactionSummary(BaseScreen):
    ...
    def check_action(self, action: str, parameters: tuple[object, ...]) -> bool | None:  # noqa: ARG002
        ...
        transaction_summary_section = self.custom_bindings.transaction_summary

        if action == transaction_summary_section.update_metadata.default_action:
            return self.node.cached.online_or_none is not None

        if action in (
            transaction_summary_section.save_transaction_to_file.default_action,
            transaction_summary_section.broadcast.default_action,
        ):
            return True
