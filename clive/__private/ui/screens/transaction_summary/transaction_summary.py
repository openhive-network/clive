...
class TransactionMetadataContainer(Horizontal):
    ...
    TRANSACTION_EXPIRATION_LABEL_INDEX: Final[int] = 3
    ...
    @property
    def expiration(self) -> Label:
        return self.query(Label)[self.TRANSACTION_EXPIRATION_LABEL_INDEX]

    @property
    def transaction_id(self) -> Label:
        return self.query(Label).last()
    ...
    async def mount_transaction_details(self, transaction: Transaction) -> None:
        ...
        things_to_mount = [
            ...
            Label(f"Transaction ID: {transaction.calculate_transaction_id()}"),
        ]
...
class CartTable(CliveCheckerboardTable):
    ...
    async def remove_item(self, event: CartItem.Delete) -> None:
        ...
        self.post_message(self.CartModified())
    ...
    def _update_cart_items_on_deletion(self, removed_item: CartItem) -> None:
        def update_indexes() -> None:
            for cart_item in self._cart_items:
                if cart_item in cart_items_to_update:
                    cart_item.operation_index = cart_item.operation_index - 1
                # update labels of all cart items
...
class TransactionSummary(BaseScreen):
    ...
    def action_load_transaction_from_file(self) -> None:
        notify_text = (
            "Loading transaction from file will clear current cart content." if len(self.profile.cart) > 0 else None
        )
        self.app.push_screen(SelectFile(notify=notify_text))
    ...
    async def load_transaction_from_file(self, event: SelectFile.Saved) -> None:
        ...
        self.profile.cart.fill_cart_from_transaction(self.transaction)
        self.app.trigger_profile_watchers()
    ...
    async def _handle_action_buttons_and_bindings(self) -> None:
        if len(self.profile.cart) <= 0:
            ...
            self.unbind("f6")
            self.unbind("f2")
        elif self.button_save.disabled and len(self.profile.cart) > 0:
