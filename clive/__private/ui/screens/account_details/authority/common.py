...
class AuthorityHeader(Horizontal):
    ...
    def compose(self) -> ComposeResult:
        if not self._memo_header:
            ...
            yield Static(f"{self._last_column_header_title}", classes=f"{CLIVE_ODD_COLUMN_CLASS_NAME} action")
        else:
            yield Static("Key", classes=f"{CLIVE_ODD_COLUMN_CLASS_NAME} memo-key")
            yield Static(f"{self._last_column_header_title}", classes=f"{CLIVE_EVEN_COLUMN_CLASS_NAME} action")
...
class AuthorityRoleCollapsibleBase(CliveCollapsible, AbstractClassMessagePump):
    ...
    Args:
        *children: Children widgets to mount.
        authority_role: Authority role to display/modify.
