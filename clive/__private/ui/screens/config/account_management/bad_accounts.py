...
class BadAccountsTable(CliveCheckerboardTable):
    ...
    def __init__(self) -> None:
        ...
        self._bad_accounts_list = AccountManager.BAD_ACCOUNT_NAMES
    ...
    def create_static_rows(self) -> Sequence[CliveCheckerboardTableRow]:
        ...
        return [
            ...
            for account in self._bad_accounts_list[start_index:end_index]
        ]
    ...
    def _get_last_page_index(self) -> int:
        return len(self._bad_accounts_list) // self.MAX_ACCOUNTS_ON_PAGE
    ...
    async def _mount_new_rows(self) -> None:
        if len(self._bad_accounts_list) == 0:
...
class BadAccounts(TabPane):
    ...
    async def search_pattern_in_list(self) -> None:
        if not CliveValidatedInput.validate_many(self.query_one(AccountNamePatternInput)):
            return

        pattern = self.query_one(AccountNamePatternInput).value_or_error
