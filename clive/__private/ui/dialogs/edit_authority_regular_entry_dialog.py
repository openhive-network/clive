...
class EditAuthorityRegularEntryDialog(
    CliveActionDialog[AuthorityEntryAccountRegular | AuthorityEntryKeyRegular | None]
):
    """
    ...
    async def _perform_confirmation(self) -> bool:
        ...
        if self._role.has(self._entry.value, new_weight):
            self.notify(f"{self._role.level} role already has entry like this one.", severity="warning")
            return False

        self._role.replace(self._entry.value, new_weight)
        assert self._role.has(self._entry.value, new_weight), "Entry was not modified."
