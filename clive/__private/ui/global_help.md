...
* on `Windows`: `Ctrl+C` and `Ctrl+V` or `select text` and mouse `right click` to paste.
* on `Linux`: `Ctrl+Shift+C` and `Ctrl+Shift+V` or `select text` and click `mouse scroll` to paste.
