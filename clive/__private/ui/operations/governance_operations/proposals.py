    ...
    async def move_row_to_actions(self) -> None:
        ...
        if self.governance_checkbox.value:
            await proposal_actions.mount_proposal(
                self.row_data.proposal_id, vote=not self.row_data.voted, pending=self.is_operation_in_cart
            )
            return
        await proposal_actions.unmount_proposal(self.row_data.proposal_id)
...
class ProposalsActions(GovernanceActions):
    ...
    async def mount_proposal(self, proposal_id: int, vote: bool = False, pending: bool = False) -> None:
        # check if proposal is already in the list, if so - return
        with contextlib.suppress(NoMatches):
            self.query_one(self.get_action_id(str(proposal_id)))
            return

        await self.mount(ProposalActionRow(proposal_id, vote=vote, pending=pending))

        if not pending:
            self.add_to_actions(str(proposal_id), vote)

    async def unmount_proposal(self, proposal_id: int) -> None:
        try:
            await self.query_one(self.get_action_id(str(proposal_id))).remove()
        except NoMatches:
            return

        self.delete_from_actions(str(proposal_id))
    ...
    def show_elements(self) -> ComposeResult:
        if self.elements_to_display is not None:
            for id_, proposal in enumerate(self.elements_to_display):
                if id_ % 2 == 0:
                    yield Proposal(proposal, table_type="ProposalsTable")
                else:
                    yield Proposal(proposal, table_type="ProposalsTable", evenness="odd")
...
class ProposalsTable(GovernanceTable):
    ...
    def create_header(self) -> GovernanceListHeader:
        return ProposalsListHeader(table_selector=self.__class__.__name__)

    def define_list_type(self) -> type[GovernanceListWidget[ProposalData]]:  # type: ignore[override]
