    ...
    def sync_data(self, content: SavingsData) -> None:
        ...
        with self.app.batch_update():
            ...
            self.interest_data_container.mount_all(
                [
                    self.create_dynamic_label(get_interest_date, "interest-payment-date"),
                    self.create_dynamic_label(get_estimated_interest, "interest-last-value"),
                    self.create_dynamic_label(get_interest_rate_for_hbd, "interest-rate-value"),
                ]
            )
...
class PendingTransfers(CliveCheckerboardTable):
    ...
    def create_dynamic_rows(self, content: SavingsData) -> list[PendingTransfer]:
        ...
        self._title.update(f"Current pending transfers (number of pending transfers: {len(content.pending_transfers)})")  # type: ignore[attr-defined]
...
class SavingsTransfers(TabPane, OperationActionBindings):
    ...
    def compose(self) -> ComposeResult:
        with ScrollablePart():
            ...
            yield SavingsBalances(self.app.world.profile_data.working_account, classes="savings-balances-on-transfer")
