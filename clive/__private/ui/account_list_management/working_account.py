...
class WorkingAccountChange(Vertical, CliveWidget):
    ...
    def __init__(self) -> None:
        ...
        self._working_account_input = AccountNameInput(
            required=False, validators=SetWorkingAccountValidator(self.profile_data)
        )
        self._previous_working_account = self.working_account

    def compose(self) -> ComposeResult:
        with Section("Change working account" if _has_working_account(self.profile_data) else "Set working account"):
            yield self._working_account_input
            yield CliveButton(
                "Change" if _has_working_account(self.profile_data) else "Set",
                variant="success",
    ...
    async def change_working_account(self) -> None:
        ...
        self.profile_data.unset_working_account()
        self.profile_data.set_working_account(self._working_account_input.value_or_error)
        self.app.trigger_profile_data_watchers()
    ...
    def change_container_function(self) -> None:
        ...
        self.query_one(SectionTitle).update(
            "Change working account" if _has_working_account(self.profile_data) else "Set working account"
        )
        self.query_one(CliveButton).remove()
        self.query_one(SectionBody).mount(button)

    def on_mount(self) -> None:
        self.watch(self.world, "profile_data", self._working_account_changed)
