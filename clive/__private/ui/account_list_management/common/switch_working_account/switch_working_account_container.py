...
class SwitchWorkingAccountContainer(Container, CliveWidget):
    ...
    def _update_local_profile_data(self, profile_data: ProfileData) -> None:
        if self.local_profile_data.accounts.tracked_accounts_sorted == profile_data.accounts.tracked_accounts_sorted:
    ...
    def _create_radio_buttons(self, profile_data: ProfileData) -> list[AccountRadioButton | CliveRadioButton]:
        ...
        tracked_account_radios = [
            AccountRadioButton(account) for account in profile_data.accounts.tracked_accounts_sorted
        ]
        no_working_account_radio = NoWorkingAccountRadioButton(
            is_working_account_set=not profile_data.accounts.is_working_account_set()
        )
    ...
    def _get_current_state_for_selected_account(
        ...
        return (
            profile_data.accounts.working
            if profile_data.accounts.is_working_account_set()
