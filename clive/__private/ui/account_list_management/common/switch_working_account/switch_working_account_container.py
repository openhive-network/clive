...
class SwitchWorkingAccountContainer(Container, CliveWidget):
    ...
    def _update_local_profile_data(self, profile_data: ProfileData) -> None:
        if (
            self.local_profile_data.account_manager.tracked_accounts_sorted
            == profile_data.account_manager.tracked_accounts_sorted
        ):
    ...
    def _create_radio_buttons(self, profile_data: ProfileData) -> list[AccountRadioButton | CliveRadioButton]:
        ...
        tracked_account_radios = [
            AccountRadioButton(account) for account in profile_data.account_manager.tracked_accounts_sorted
        ]
        no_working_account_radio = NoWorkingAccountRadioButton(
            is_working_account_set=not profile_data.account_manager.is_working_account_set()
        )
    ...
    def _create_tracked_accounts_content(self, profile_data: ProfileData) -> CliveRadioSet | NoTrackedAccounts:
        if not profile_data.account_manager.has_tracked_accounts:
    ...
    def confirm_selected_working_account(self) -> None:
        ...
        self.profile_data.account_manager.switch_working_account(new_working_account)
    ...
    def _get_current_state_for_selected_account(
        ...
        return (
            profile_data.working_account
            if profile_data.account_manager.is_working_account_set()
            else NoWorkingAccountSelected()
        )
    ...
    def _is_current_working_account_selected(self) -> bool:
        ...
        return self.profile_data.account_manager.is_account_working(self._selected_account)
