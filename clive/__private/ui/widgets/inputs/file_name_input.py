...
class FileNameInput(TextInput):
    ...
    def update_root_path_of_file(self, root_path: Path) -> None:
        self._update_filename_validator_root_path(root_path)
        self._update_root_path_of_file(root_path)

    def _update_root_path_of_file(self, root_path: Path) -> None:
        self._root_directory_path = root_path

    def _update_filename_validator_root_path(self, root_path: Path) -> None:
        for validator in self.input.validators:
            if isinstance(validator, FilePathValidator):
                validator.update_root_path_of_file(root_path)
