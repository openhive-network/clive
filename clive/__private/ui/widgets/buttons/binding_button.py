...
class BindingButton(CliveButton):
    ...
    def __init__(
        ...
        Args:
        ----
        binding_key: binding to be displayed on button should be the same as in bindings list.
        ...
        for binding in bindings_list:
            assert isinstance(
                binding, Binding
            ), f"{self.__class__.__name__} should be used only with `Binding` classes! Got {type(binding)}"
