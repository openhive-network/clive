...
class ActivityStatsButton(DynamicOneLineButton):
    can_focus = False
    ...
    def __init__(
        ...
        self.tooltip = f"Perform this operation as {self._account.name}"
    ...
    async def push_activity_screen(self) -> None:
        if self._activity_type == "savings":
            ...
            if not self.profile_data.is_account_working(self._account):
                self.profile_data.switch_working_account(self._account)
...
class ActivityStats(TrackedAccountReferencingWidget):
    def compose(self) -> ComposeResult:
        ...
        yield ActivityStatsButton(self._account, "balance", "hive", variant="grey-darken")
        yield ActivityStatsButton(self._account, "savings", "hive", variant="grey-lighten")
        yield Static("HBD", classes="token token-variant")
        yield ActivityStatsButton(self._account, "balance", "hbd", variant="grey-lighten")
