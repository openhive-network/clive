...
class AlarmData(CliveCheckerboardTable):
    def __init__(self, alarm: Alarm[Any, Any]):
        self._alarm_titled_data = alarm.ensure_alarm_data.get_titled_data()
        super().__init__(
    ...
    def compose(self) -> ComposeResult:
        with AlarmInfo():
            ...
            yield FixAlarmInfoWidget(
                alarm=self._alarm, alarm_fix_details=self._alarm_fix_details, related_account=self._related_account
            )
