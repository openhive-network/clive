...
@dataclass
class CliveBindings:
    """Main container for all configurable key bindings in the application.

    This class manages the complete set of key bindings organized by sections.
    It provides methods for serialization/deserialization and access to bindings.
    """
    ...
    def _binding_section_fields(self) -> Generator[tuple[str, CliveBindingSection]]:
        """Yield all binding section fields and their values."""
    ...
    def _get_global_bindings_table(self) -> str:
        """Generate a formatted table of global bindings from the app section.

        Returns:
            String containing the formatted binding table
        """
        table = ""
    ...
    def _update_section(self, section_name: str, section_instance: CliveBindingSection) -> None:
        """Update a binding section with a new instance.

        Args:
            section_name: The ID of the section to update
            section_instance: The new section instance
        """
...
"""Pre-defined application bindings (default configuration)"""


def load_custom_bindings() -> CliveBindings:
    """Load user's custom bindings from the configuration file.

    Returns:
        CliveBindings instance with user's custom configuration
    """
