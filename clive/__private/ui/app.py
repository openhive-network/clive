...
class Clive(App[int]):
    ...
    def update_alarms_data_asap(self) -> Worker[None]:
        ...
        return self.run_worker(self._update_alarms_data_asap())

    async def _update_alarms_data_asap(self) -> None:
        ...
        await self._update_alarms_data_awaitable()
        self.resume_refresh_alarms_data_interval()

    def update_data_from_node_asap(self) -> Worker[None]:
        return self.run_worker(self._update_data_from_node_asap())

    async def _update_data_from_node_asap(self) -> None:
        self.pause_refresh_node_data_interval()
        await self._update_data_from_node_awaitable()
    ...
    def update_alarms_data_asap_on_newest_node_data(self) -> Worker[None]:
        ...
        return self.run_worker(self._update_alarms_data_asap_on_newest_node_data())

    async def _update_alarms_data_asap_on_newest_node_data(self) -> None:
        await self._update_data_from_node_asap()
        await self._update_alarms_data_asap()

    async def _update_alarms_data_awaitable(self) -> None:
    ...
    @work(name="alarms data update worker", group="alarms_data")
    async def update_alarms_data(self) -> None:
        await self._update_alarms_data_awaitable()

    async def _update_data_from_node_awaitable(self) -> None:
    ...
    @work(name="node data update worker", group="node_data")
