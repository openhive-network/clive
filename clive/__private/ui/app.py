...
class Clive(App[int]):
    ...
    COMMAND_PALETTE_BINDING = CLIVE_PREDEFINED_BINDINGS.glob.command_palette.key
    ...
    BINDINGS = [
        ...
        CLIVE_PREDEFINED_BINDINGS.glob.transaction_summary.create(
            action="go_to_transaction_summary", description="Transaction summary", show=False
        ),
        CLIVE_PREDEFINED_BINDINGS.glob.dashboard.create(action="go_to_dashboard", description="Dashboard", show=False),
        CLIVE_PREDEFINED_BINDINGS.glob.settings.create(action="go_to_config", description="Config", show=False),
