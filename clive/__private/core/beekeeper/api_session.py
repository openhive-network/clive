...
def api(foo: FooT) -> FooT:
    @wraps(foo)
    async def impl(this: BeekeeperApiSession, **kwargs: Any) -> Any:
...
class BeekeeperApiSession:
    def __init__(self, owner: BeekeeperSession, communication: Communication | None = None) -> None:
        self._owner = owner
        self._communication = communication or Communication()
