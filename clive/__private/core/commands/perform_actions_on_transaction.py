    ...
    async def _execute(self) -> None:
        ...
        if self.sign_keys is not None or self.autosign:
            ...
            if self.autosign:
                ...
                # Sign with key(s) - use multisign mode only if multiple keys AND transaction is not signed yet
                # If transaction is already signed, error should be raised regardless of number of keys
                already_signed_mode = (
                    "multisign" if len(keys_to_sign) > 1 and not transaction.is_signed else self.already_signed_mode
                )
