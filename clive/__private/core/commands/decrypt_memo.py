...
class DecodeEncryptedMemoError(CommandError):
    def __init__(self, command: Command) -> None:
        super().__init__(command, "Failed to decode memo.")


class DecryptMemoError(CommandError):
    def __init__(self, command: Command) -> None:
        super().__init__(command, "Failed to decrypt the memo.")
    ...
    async def _execute(self) -> None:
        ...
        try:
            ...
            if "Decryption failed" in str(error):
                raise DecryptMemoError(self) from error
            raise
