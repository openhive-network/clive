...
class LoadTransaction(CommandWithResult[Transaction]):
    ...
    async def _execute(self) -> None:
        ...
        with contextlib.suppress(iwax.WaxOperationFailedError, JSONDecodeError, ValidationError):
            deserialized_transaction = Transaction(**iwax.deserialize_transaction(self.file_path.read_bytes()).dict())
            if isinstance(deserialized_transaction, Transaction):
                self._result = deserialized_transaction
                return
