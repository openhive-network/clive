...
class _AccountAlarmsHarvestedData:
    ...
    owner_history: ListOwnerHistories
...
class UpdateAlarmsData(
    ...
    async def _harvest_data_from_api(self) -> AccountHarvestedAlarmsDataContainer:
        ...
        async with self.node.batch() as node:
            for account in self.accounts:
                ...
                owner_history = await node.api.database_api.list_owner_histories(
                    start=(account.name, _get_utc_epoch()), limit=1
                )

                accounts_data[account] = _AccountAlarmsHarvestedData(
                    decline_voting_rights, change_recovery_account_requests, owner_history
                )
    ...
    async def _sanitize_data(self, data: AccountHarvestedAlarmsDataContainer) -> AccountSanitizedAlarmsDataContainer:
        ...
        for account, unsanitized in data.items():
            account_sanitized_data[account] = _AccountAlarmsSanitizedData(
                ...
                owner_history=self._sanitize_owner_history(unsanitized.owner_history, account.name),
            )
    ...
    def _sanitize_owner_history(
        self, owner_key_change_in_progress: ListOwnerHistories, account_name: str
    ) -> list[OwnerHistory]:
        data = owner_key_change_in_progress.owner_auths
        self._assert_account_name_matches_in_results(data, account_name, "account")
        return data
