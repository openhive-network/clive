...
class ChainDataRetrieval(CommandDataRetrieval[HarvestedDataRaw, SanitizedData, ChainData]):
    ...
    async def _harvest_data_from_api(self) -> HarvestedDataRaw:
        async with await self.node.batch() as node:
            ...
            witness_schedule = await node.api.database.get_witness_schedule()
            version = await node.api.database.get_version()
            hardfork_properties = await node.api.database.get_hardfork_properties()
            current_price_feed = await node.api.database.get_current_price_feed()
            feed = await node.api.database.get_feed_history()
