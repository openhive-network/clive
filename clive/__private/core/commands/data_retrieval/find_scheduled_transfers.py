    ...
    async def _harvest_data_from_api(self) -> _HarvestedDataRaw:
        async with await self.node.batch() as node:
            return _HarvestedDataRaw(
                recurrent_transfers=await node.api.database.find_recurrent_transfers(from_=self.account_name),
                account_data=await node.api.database.find_accounts(accounts=[self.account_name]),
            )
