...
class Commands(Generic[WorldT_co]):
    ...
    async def create_wallet(
        ...
        password: Password later used to unlock the wallet. If None, will be generated by the beekeeper.
        """
    ...
    async def is_wallet_unlocked(self, *, wallet: AsyncWallet | None = None) -> CommandWithResultWrapper[bool]:
        ...
        return await self.__surround_with_exception_handlers(
            IsWalletUnlocked(
                wallet=wallet if wallet is not None else self._world._unlocked_wallets_ensure.user_wallet,
            )
        )
        ...
        *,
        ...
        return await self.__surround_with_exception_handlers(
            PerformActionsOnTransaction(
                ...
                unlocked_wallet=self._world._unlocked_wallets_ensure.user_wallet if sign_key else None,
                sign_key=sign_key,
    ...
    async def sync_state_with_beekeeper(self, source: LockSource = "unknown") -> CommandWrapper:
        return await self.__surround_with_exception_handlers(
            SyncStateWithBeekeeper(
                wallet=self._world._unlocked_wallets_ensure.user_wallet,
                profile_encryption_wallet=self._world._unlocked_wallets_ensure.encryption_wallet,
                app_state=self._world.app_state,
                source=source,
            )
        )
