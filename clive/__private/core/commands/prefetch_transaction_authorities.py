...
from clive.__private.core.iwax import transaction_get_impacted_accounts
...
class PrefetchTransactionAuthorities(CommandWithResult[dict[str, Account]]):
    """
    Prefetches account authority data needed for transaction signing.

    Fetches accounts in breadth-first, batched layers up to HIVE_MAX_SIG_CHECK_DEPTH,
    following account_auths references at each level.

    Attributes:
        transaction: The transaction to analyze for impacted accounts.
    ...
    async def _execute(self) -> None:
        impacted = transaction_get_impacted_accounts(self.transaction)
        await self._fetch_and_cache(impacted)

        previous_layer = impacted
