...
class CliveAuthority:
    ...
    @property
    def active_role(self) -> CliveAuthorityRoleWrapper:
        return CliveAuthorityRoleWrapper(self._operation.roles.active)

    @property
    def memo_role(self) -> CliveAuthorityRoleWrapper:
        return CliveAuthorityRoleWrapper(self._operation.roles.memo)

    @property
    def owner_role(self) -> CliveAuthorityRoleWrapper:
        return CliveAuthorityRoleWrapper(self._operation.roles.owner)

    @property
    def posting_role(self) -> CliveAuthorityRoleWrapper:
        return CliveAuthorityRoleWrapper(self._operation.roles.posting)

    @property
    def roles(self) -> list[CliveAuthorityRoleWrapper]:
        return [self.active_role, self.owner_role, self.posting_role, self.memo_role]

    def get_entries(self) -> list[CliveAuthorityEntryWrapper]:
        """
...
class CliveAuthorityWrapper(CliveAuthorityBase):
    ...
    @property
    def account_entries(self) -> list[CliveAuthorityEntryWrapper]:
        return [
            CliveAuthorityEntryWrapper(account_entry, weight)
            for account_entry, weight in self._authority.account_auths.items()
        ]

    @property
    def key_entries(self) -> list[CliveAuthorityEntryWrapper]:
        return [
            CliveAuthorityEntryWrapper(key_entry, weight) for key_entry, weight in self._authority.key_auths.items()
        ]

    @property
    def all_entries(self) -> list[CliveAuthorityEntryWrapper]:
    ...
    def is_match(self, *patterns: str) -> bool:
        ...
        """
        return any(entry_wrapper_object.is_match(*patterns) for entry_wrapper_object in self.all_entries)

    def collect_all_entry_values(self) -> list[str]:
    ...
    def collect_weights(self, keys: KeyManager) -> list[int]:
        ...
        """
        return [self._authority.key_auths[key] for key in self._authority.key_auths if key in keys]
